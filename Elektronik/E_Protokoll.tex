...
\begin{equation}
\sigma_{\delta_n}=\delta_n \sqrt{(\frac{\sigma_{x_n}}{x_n \ln{x_n}})^2+(\frac{\sigma_T}{T})^2} \quad \text{mit} \quad x=\frac{A_n}{A_{n+a}} \rightarrow \sigma_x=x\cdot \sqrt{(\frac{\sigma_{A_n}}{A_n})^2+(\frac{\sigma_{A_{n+1}}}{A_{n+1}})^2}
\end{equation}
...
Mittels Mittelwert und Fehler auf diesen ergibt sich für jede der fünf Messungen pro eingestelltem Widerstand eine mittlere Periodendauer mit Fehler. Durch gewichteten Mittelwert werden diese zu einem Endergebnis für jede Widerstandseinstellung zusammengefasst.\\
...
Eine Zusammenstellung der auf diese Weise gewonnen Frequenzen bei diversen Widerstandseinstellungen finden sich in Tabelle \ref{tab:Frequenzen}. Für noch größere Widerstände lieferte die Bestimmung der Frequenz auf diese Weise keine sinnvollen Ergebnisse.
...
Es wurde versucht zwei Einhüllende der Form $U(t)=U_0 e^{-\delta (t-t_0)}\cdot (1+\delta (t-t_0))$ anzupassen, so dass der gemessene Verlauf möglichst innerhalb der Einhüllenden verbleibt. Der Parameter $t_0$ ist so zu wählen, dass dem verspäteten Messstart aufgrund des Triggers Rechnung getragen wird. Die beste Einhüllung ergab sich für $R=185\Omega$ und $R=200\Omega$, wobei $L=35.8mH$ verwendet wurden. (vgl. Abb.\ref{fig:Aperiodisch} )
...
\end{center}
\caption{Zoom auf zwei Bereiche. Das gemessene Spannungssignal ist durchgezogen gezeichnet. Im zweiten Bild ist der einzige Bereich gezeigt indem das Einhüllen nicht gut gelingt.}
...
\begin{equation}
R_{ap}=2\cdot \sqrt{\frac{L}{C}}=2\cdot \sqrt{\frac{35.8mH}{4.7\mu F}}=174.6\Omega
\end{equation}

\begin{equation}
\sigma_{R_{ap}}=\sqrt{(\frac{\sigma_L^2}{CL})^2+(\frac{\sigma_C^2 L}{C^3})^2}=?
\end{equation}

Die große Abweichung von rund 4.2 Standardabweichungen zum erwarteten Wert ist wohl darauf zurückzuführen, dass es sich bei dem Spannungsverlauf für den Einhüllende bestimmt wurden nur um die beste Approximation des aperiodischen Grenzfalls handelt die in der verbliebenen Zeit noch möglich war. Bei genauerem Abtasten des entscheidenden Bereichs wäre zu erwarten, dass das Ergebnis weniger stark abweicht.
