...
kalibration_a = 100./(t_kochen-t_eis)
kalibration_b = -kalibration_a*t_eis

temperatur = lambda x: kelvin(kalibration_a*x + kalibration_b)



# Raumtemperatur
raumtemp_messung = 0
t0 = p.lese_lab_datei("{0:s}/raumtemp/{1:d}.lab".format(Gruppe, raumtemp_messung))[0:3]
...
plt.plot(x, a*x+b)
plt.figtext(0.15, 0.75, "y = ax + b\na = ({0:1.3f} $\pm$ {1:1.3f})*1e-9 V/K^4  ".format(a*1e9, ea*1e9))
...
epsilon = p_gemessen(t)/p_ideal(u)
...
odr    = odr.ODR(data, model, beta0=[-1e-4*t0**4, a, 4.])
output = odr.run()
ndof = len(x)-2
...
plt.plot(t, f(par, t))
plt.figtext(0.15, 0.6, "y = a + b x^c \na = ({0:1.3f} $\pm$ {1:1.3f}) V\nb = ({2:1.3f} $\pm$ {3:1.3f}) V/K\nc = ({4:1.3f} $\pm$ {5:1.3f})\n$\chi^2/ndof$ = {6:1.1f}".format(par[0], sd[0], par[1], sd[1], par[2], sd[2], chi2/ndof))
plt.ylabel("Spannung [V]")
plt.xlabel("")
