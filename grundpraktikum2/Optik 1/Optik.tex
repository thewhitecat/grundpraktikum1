...
\begin{equation}
\sigma_{\lambda_{sys}} = \sqrt{\left(\dfrac{\sqrt{\dfrac{a}{n-b}}}{2\cdot a}\cdot \sigma_{a}\right)^2 + \left(\dfrac{a}{2\cdot (n-b)^2\cdot \sqrt{\dfrac{a}{n-b}}} \cdot \sigma_b\right)^2}
\end{equation}
\begin{equation}
\sigma_{\lambda_{stat}} = \sqrt{ \left(\dfrac{a}{2\cdot (n-b)^2\cdot \sqrt{\dfrac{a}{n-b}}} \cdot \sigma_n \right)^2}
\end{equation}
...
\hline
Lit.[nm] & 636.23 & 481.05 & 472.22 & 468.01\\
\hline
$\sigma$ & 0.30 & 1.64 & 0.49 & 0.27\\
\hline
\end{tabular}
\caption{Ergebnisse der Wellenlängenbestimmung und Vergleich mit den Literaturwerten. Angegeben wurde zuerst der statistische Fehler und dann der systematische. Es wurde außerdem die Anzahl der Standardabweichungen zwischen den Werten angegeben.}
...
Ein Gitter ist ein optisches Bauteil, das einfallendes Licht beugt. Das Beugungsmuster ist ähnlich dem der Beugung an einem Doppelspalt. Allerdings hat das Gitter gegenüber dem Doppelspalt einige Vorteile: Durch die hohe Anzahl der Spalte ist die durchgelassene Intensität deutlich höher als bei einem Doppelspalt. Zudem ermöglicht dies schmalere Spalte, welches eine besser Auflösung ermöglicht.\\
	...
	\begin{figure}
		\begin{center}
			\includegraphics[scale=0.6]{Bilder/Schematischer_Aufbau_Gitterspektrometer.PNG}
		\end{center}
	...
	\begin{equation*}
	\bar{\alpha} = 84^{\circ} \; 32,5' \qquad und \qquad \sigma_{\alpha} = 1,08'
	\end{equation*}
	...
	\begin{equation}
	\sigma _{\theta} = \sqrt{\left( \dfrac{\sigma_\alpha}{\sqrt{10}} \right)^2 + \left( \dfrac{\sigma_\alpha}{\sqrt{2}} \right)^2} = \sigma_\alpha \cdot \sqrt{\dfrac{6}{10}} 
	\end{equation}
